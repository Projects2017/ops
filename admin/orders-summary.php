...
?><br>
<table id="ordersummary" border="0" cellspacing="0" cellpadding="5" width="90%" align="center"<?php
	...
	<td colspan="9" class="text_12" align="center" style="vertical-align: bottom;">
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
			...
			<select id="f_type" name="f_type">
				<option value=""<?php if ($filter['type'] == '') echo ' SELECTED'; ?>>All</option>
				<option value="|o|f"<?php if ($filter['type'] == '|o|f') echo ' SELECTED'; ?>><?php echo getOrderType('o'); ?> or <?php echo getOrderType('f'); ?></option>
				<option value="o"<?php if ($filter['type'] == 'o') echo ' SELECTED'; ?>><?php echo getOrderType('o'); ?></option>
				<option value="f"<?php if ($filter['type'] == 'f') echo ' SELECTED'; ?>><?php echo getOrderType('f'); ?></option>
				<option value="c"<?php if ($filter['type'] == 'c') echo ' SELECTED'; ?>><?php echo getOrderType('c'); ?></option>
			</select>
...
<?php } /* end if !$nopages */ ?>
  <tr bgcolor="#fcfcfc"> 
	...
	<?php if (secure_is_superadmin()) { ?><td bgcolor="#CCCC99" class="fat_black_12">DO</td>
	<td bgcolor="#CCCC99" class="fat_black_12">Print</td><?php } ?>
	-->
...
?>
    <tr valign="top"> 
      <td bgcolor="#FFFFFF" class="text_12"><?php echo date('m/d/Y h:ia', strtotime($result['ordered'])); ?></td>
      ...
      <td bgcolor="#FFFFFF" class="text_12"><?php echo getUserName($result['snapshot_user']); ?></td>
      <td bgcolor="#FFFFFF" class="text_12" align="right"><?php echo makeThisLookLikeMoney($result['total']); ?></td>
      <td bgcolor="#FFFFFF" class="text_12" align="center"><a href="viewpo.php?po=<?php echo $result['ID']+1000; ?>" target="order_window">view&nbsp;<?php echo getOrderType($result['type']); ?></a><?php if ($result['type'] == 'o'): ?>&nbsp;<a href="viewpo.php?po=<?php echo $result['ID']+1000; ?>&for=vendor" target="order_window">[V]</a><?php endif; ?></td>
	  ...
	  <td bgcolor="#FFFFFF" class="text_12">
		...
		<?php if (secure_is_superadmin()) { ?>
			...
			<td bgcolor="#FFFFFF" class="text_12">
				<?php if ($result['printedonsummary'] != 'Y') { ?><a OnClick="print_order(this,<?php echo $po; ?>);return(false);"><img border=0 src="/images/print.gif" alt="Print"></a><?php } else { ?>&nbsp;<?php } ?>
			</td>
		<?php } ?>
    </tr>
<?php
