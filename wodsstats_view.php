...
if ($report_type == "display") { /* print report */
	...
	$gWodsStats->GetSingleSum($user_id, strtotime($select_date), strtotime($select_date), 'day');
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td><p><select name="user_id">
		...
		$sql = "SELECT DISTINCT users.ID, users.first_name, users.last_name FROM users INNER JOIN 
		 wodsstats ON users.ID=wodsstats.user_id WHERE users.wodsable = 'Y' AND users.nonPMD != 'Y' AND users.disabled != 'Y'  AND wodsstats.stat_date LIKE '$select_date%'";
...
<table border="0" cellspacing="0" cellpadding="5" width="760">
  <tr bgcolor="#FFFFFF"> 
    ...
    <td><p>
	<?php
	$sql = "SELECT first_name, last_name FROM users WHERE ID=$user_id AND users.wodsable = 'Y' AND nonPMD != 'Y' AND disabled != 'Y'";
	$query = mysql_query($sql);
	checkDBError();
	if (mysql_num_rows($query) > 0)
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
    <tr bgcolor="#FFFFFF"> 
      ...
      <td><p>
        ...
        <?php
		$sql = "SELECT DISTINCT stat_date FROM wodsstats WHERE user_id=$user_id ORDER BY stat_date DESC";
		$query = mysql_query($sql);
		checkDBError();
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
    <tr bgcolor="#FFFFFF"> 
      ...
      <td><p><select name="user_id">
		<?php
		$sql = "SELECT ID, first_name, last_name FROM users WHERE users.wodsable = 'Y' AND nonPMD != 'Y' AND disabled != 'Y' ORDER BY last_name, first_name";
		$query = mysql_query($sql);
		checkDBError();
		if (mysql_num_rows($query) > 0)
			while ($result = mysql_fetch_array($query)) {
