...
function getdealerstats($userid) {
	...
	$return = array();
	// Bedding
	$sql = 'SELECT COUNT(`order_forms`.`ID`) AS `count`, SUM(`order_forms`.`total`) AS `total` FROM `order_forms` INNER JOIN `forms` ON `order_forms`.`form` = `forms`.`ID` INNER JOIN `vendors` ON `forms`.`vendor` = `vendors`.`ID` WHERE `order_forms`.`user` = "'.$userid.'" AND `order_forms`.`deleted` = 0 AND `order_forms`.`type` = "o" AND `vendors`.`Access_type` = "Bedding" AND DATE_SUB(CURDATE(),INTERVAL 45 DAY) <= `order_forms`.`ordered`;';
	...
	// Case Goods
	$sql = 'SELECT COUNT(`order_forms`.`ID`) AS `count`, SUM(`order_forms`.`total`) AS `total` FROM `order_forms` INNER JOIN `forms` ON `order_forms`.`form` = `forms`.`ID` INNER JOIN `vendors` ON `forms`.`vendor` = `vendors`.`ID` WHERE `order_forms`.`user` = "'.$userid.'" AND `order_forms`.`deleted` = 0 AND `order_forms`.`type` = "o" AND `vendors`.`Access_type` IN ("Case Goods", "Upholstery") AND DATE_SUB(CURDATE(),INTERVAL 45 DAY) <= `order_forms`.`ordered`;';
	$query = mysql_query($sql);
	checkDBerror($sql);
	$temp = mysql_fetch_assoc($query);
...
<table class="sortable" id="users" border="0" cellspacing="0" cellpadding="5" align="left">
  <tr class="skiptop">
	...
	Division: <select id="f_division" name="f_division">
		...
		<option value="1" <?php if ($filter['division'] == '1') echo "SELECTED"; ?>>1</option>
		<option value="2" <?php if ($filter['division'] == '2') echo "SELECTED"; ?>>2</option>
	</select>
	<br/>
	E-Mail: <input type=text value="<?php echo $filter['email']; ?>" name="f_email">
	State: <input type=text size=2 value="<?php if ($filter['state'] != '0') echo $filter['state']; ?>" name="f_state">
	<input type=submit value="Filter"><br />
	Date Range: Previous 45 days</form>
    </td>
  </tr>
  <tr> 
  ...
  <tr> 
	...
	?>
	<td class="text_12" style="text-align: right"><?php
  ...
  <?php } ?>
  <tr class="sortbottom">
    <td colspan="6" class="fat_black_12" bgcolor="#fcfcfc">Total Dealers Displayed: <?php echo $numrows; ?></td>
	<td class="fat_black_12"  bgcolor="#fcfcfc" style="text-align: right"><?php
