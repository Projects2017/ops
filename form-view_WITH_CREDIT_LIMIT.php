...
<?php
/* BEGIN CREDIT CHECK */
$query = mysql_query("SELECT credit_limit FROM users WHERE ID=$userid");
checkDBError();
if ($result = mysql_fetch_array($query))
	$credit_limit = $result[0];
$query = mysql_query("SELECT SUM(total) FROM order_forms WHERE user=$userid AND deleted=0");
checkDBError();
if ($result = mysql_fetch_array($query))
	$balance = $result[0];

//echo "<p>credit_limit = $credit_limit, balance = $balance</p>";

if ($balance >= $credit_limit)
	echo "<p class=\"alert\">Your credit limit has been exceeded, please make a payment immediately.
	  If you feel this message is the result of an error, please contact Amy Bowen at 614-538-0675.</p>
		<p><b>[<a href=\"selectvendor.php\">Back</a>]</b></p>";
else {
	/* END CREDIT CHECK */
	...
	<table width="700" border="0">
	  <tr valign="top">
		...
		<td align="right"><a href="" onclick="window.print(); return false;">Print</a> &nbsp;&nbsp;<font color="#0000FF"><span class="text_12">Minimum:
		<b><?php echo $minimum ?></b></span></font></td>
	</tr>
