    ...
    <tr class="skiptop">
        <td colspan="20" class="text_12" align="center" style="vertical-align: bottom;">
            <form action="<? echo $_SERVER['PHP_SELF']; ?>" method="get">
                ...
                Merchant PO: <input type="text" value="<? echo $filter['merchantpo']; ?>" id="f_merchantpo" name="f_merchantpo" size="6">
                Order: <input type="text" value="<? echo $filter['orderid']; ?>" id="f_orderid" name="f_orderid" size="6">
                POs per Page:
...
//while ($result2 = mysql_fetch_assoc($query2)) { // Start BoL While
    ...
    ?>
    <tr valign="top">
        <td bgcolor="#FFFFFF" class="text_12" rowspan="<?= $num_bols ?>"><? echo date('m/d/Y h:ia', strtotime($result['ordered'])); ?></td>
        <td bgcolor="#FFFFFF" class="text_12" rowspan="<?= $num_bols ?>"><a href="viewpo.php?po=<? echo $result['ID']+1000; ?>" target="order_window" class="text_12"><? echo $po; ?></a></td>
        <td bgcolor="#FFFFFF" class="text_12" rowspan="<?= $num_bols ?>"><? echo $result['merchantpo']; ?></td>
        <td bgcolor="#FFFFFF" class="text_12" rowspan="<?= $num_bols ?>"><? echo $result['orderid']; ?></td>
        <? /* Do First BoL */
        $result2 = array_shift($bols);
        if ($result2):
            ?>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12">&nbsp;</td>
            <td bgcolor="<?= $result2['displaycolor'] ?>" align="center"><a href="/shipping/viewbol.php?id=<?= $result2['ID'] ?>" target="order_window" class="text_12"><?=$result2['displaystring'] ?></a></td>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"><?= htmlentities($result2['carrier']) ?></td>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"><?= htmlentities($result2['trackingnum']) ?></td>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"><?= htmlentities($result2['shipdate']) ?></td>
        <? else: ?>
        ...
        {
            ?><td class="text_12" style="background-color: <?
        ...
        ?>><a href="../form.php?action=view&form=furniture&viewid=<?= $poclaims[0]['id'] ?>" style="color: <? if($poclaims[0]['status']=='9') { echo 'white'; } else { echo 'black'; } ?>"><?= $poclaims[0]['id'] ?></a><?
        }
        else
        {
        ?><td bgcolor="#FFFFFF">&nbsp;<?
            ...
            ?></td>
        <td bgcolor="#FFFFFF" colspan="5" rowspan="<?= $num_bols ?>" class="text_12">
            <a href="ch_summary.php?claim_redirect=<?= $result['ID']+1000; ?>">
    ...
    foreach ($bols as $result2)
    {
        ?>
        <tr valign="top">
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"<?
            if($current_line == $totalbols) { ?> rowspan="<?= count($poclaims)-$totalbols > 0 ? count($poclaims)-$totalbols : "1" ?>"<? } ?>>&nbsp;</td>
            <td bgcolor="<?= $result2['displaycolor'] ?>" align="center"<?
            if($current_line == $totalbols) { ?> rowspan="<?= count($poclaims)-$totalbols > 0 ? count($poclaims)-$totalbols : "1" ?>"<? } ?>><a href="/shipping/viewbol.php?id=<?= $result2['ID'] ?>" target="order_window" class="text_12"><?=$result2['displaystring'] ?></a></td>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"<?
            if($current_line == $totalbols) { ?> rowspan="<?= count($poclaims)-$totalbols > 0 ? count($poclaims)-$totalbols : "1" ?>"<? } ?>><?= htmlentities($result2['carrier']) ?></td>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"<?
            if($current_line == $totalbols) { ?> rowspan="<?= count($poclaims)-$totalbols > 0 ? count($poclaims)-$totalbols : "1" ?>"<? } ?>><?= htmlentities($result2['trackingnum']) ?></td>
            <td bgcolor="<?= $result2['bgcolor'] ?>" class="text_12"<?
            if($current_line == $totalbols) { ?> rowspan="<?= count($poclaims)-$totalbols > 0 ? count($poclaims)-$totalbols : "1" ?>"<? } ?>><?= htmlentities($result2['shipdate']) ?></td>
            <?
            if(count($poclaims)>$current_line)
            {
                ...
                ?>><a href="../form.php?action=view&form=furniture&viewid=<?= $poclaims[$current_line]['id'] ?>" style="color: <? if($poclaims[$current_line]['status']=='9') { echo 'white'; } else { echo 'black'; } ?>"><?= $poclaims[$current_line]['id'] ?></a><?
            }
            else
            {
            ?><td bgcolor="#FFFFFF">&nbsp;<?
